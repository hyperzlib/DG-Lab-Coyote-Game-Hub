...
import CoyoteBluetoothPanel from './CoyoteBluetoothPanel.vue';
...
<template>
    ...
    <div class="flex gap-8 mb-4 w-full">
        ...
        <div class="opacity-60 text-right">
            强度范围：{{ parentState.strengthVal }} - {{ parentState.strengthVal + parentState.randomStrengthVal }}，强度上限请在DG-Lab中设置
        </div>
    </div>
    <div class="flex items-center gap-2 lg:gap-8 mb-4 w-full">
        <label class="font-semibold w-30">B通道</label>
        <ToggleButton v-model="parentState.bChannelEnabled" onIcon="pi pi-circle-on" onLabel="已启用" offIcon="pi pi-circle-off"
            offLabel="已禁用" />
    </div>
    <div class="w-full flex flex-col md:flex-row items-top lg:items-center gap-2 lg:gap-8 mb-8 lg:mb-4">
        <label class="font-semibold w-30">B通道强度倍数</label>
        <InputNumber class="input-small" :disabled="!parentState.bChannelEnabled" v-model="parentState.bChannelMultiple" />
        <div class="flex-grow flex-shrink"></div>
    </div>
