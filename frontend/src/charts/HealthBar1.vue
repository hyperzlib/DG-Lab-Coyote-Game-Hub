...
const hp = computed(() => ({
    low: Math.max(0, props.valLimit - props.valHigh),
    high: Math.max(0, props.valLimit - props.valLow),
}));
...
const heartValueList = computed(() => { // 计算每个心心的红黄色值
    ...
    for (let i = 0; i < heartNum.value; i++) {
        heartValueList.push({
            valRed: Math.min(100, Math.max(0, (hp.value.low - i * 4) * 100 / 4)),
            valYellow: Math.min(100, Math.max(0, (hp.value.high - i * 4) * 100 / 4)),
        });
    }
    ...
    <div class="bar-chart">
        ...
        <div class="mt-2 pr-2 flex justify-between items-center value-text" :class="{ 'text-inverted': props.textInverted, 'hidden': props.hideText }">
            <div class="flex gap-2 items-center">
                <span>HP:</span><span>{{ hp.low }}/{{ props.valLimit }}</span><span>(+{{ props.randomStrength }})</span>
            </div>
            <div>
